...
- Use `log/slog` for structured logging. Favor `Logger.InfoContext` or `Logger.ErrorContext` to ensure context propagation for tracing.
...
- Validate input data
- When parsing API requests, prefer `json.NewDecoder(r.Body).Decode(&v)` over `io.ReadAll` + `json.Unmarshal` to save on memory allocations.
